...
### Tests
You will write your tests in the `tests` folder inside the `analysis` folder, and then write as many test commands as you need in the `.travis.yml` file that is provided with your template. For language specific templates, example test commands are provided.
...
analysis/results/submission ---->          /analysis/results/submission       # Where the user writes final submission data
...
## Instructions
This is easy! The competition already has beautiful instructions and documentation, so we will provide that in the base README.md. Note that if you do this, you should link to the original source to give credit.
...
### Small Data
For small data, you **could** download directly into the container during build, but we recommend that you add the data to your Github repository, and then add to the container in the `%files` section. If, for any reason, the download link doesn't work, you can rest assured that you have the data. For example, for our mu tau tau tau competition, we will provide the data, and thus download it into `/data/input`. Note that I am doing this on my host.

```
cd data/input
wget https://www.kaggle.com/c/flavours-of-physics/download/check_agreement.csv.zip
wget https://www.kaggle.com/c/flavours-of-physics/download/check_correlation.csv.zip
wget https://www.kaggle.com/c/flavours-of-physics/download/sample_submission.csv.zip
wget https://www.kaggle.com/c/flavours-of-physics/download/test.csv.zip
wget https://www.kaggle.com/c/flavours-of-physics/download/training.csv.zip
```

If these were extracted, I could just add them in files. But since I want to extract and add at build time, I'm going to add them to the `%setup` section. This way, they will be added extracted into the container before `%post`, and the ones shared in the repository on GIthub will be the compressed files:
...
and note under files, all the data in input is copied into the container
...
## Code
Since this is a pre-existing competition, we are lucky that the evaluation metrics have been well thought out **and** implemented! This, we will take the same approach as we did with data - adding the files to our repo, and then they will be added to the container automatically (no need to unzip for these small ones).

```
cd analysis
wget https://raw.githubusercontent.com/yandexdataschool/flavours-of-physics-start/master/evaluation.py
```

Next I am going to add a `main.py`, and I will base this on the [baseline example](https://github.com/yandexdataschool/flavours-of-physics-start/blob/master/baseline.ipynb). The one difference will be that instead of having the users load and save data, I'm going to turn these into functions in helpers, import them into main, and have the user just run them. This gives me more control over how and where the data is saved. You can see the final helpers functions that I derived in [this folder](https://github.com/containers-ftw/flavours-of-physics-ftw/tree/master/analysis/helpers), and thus the user can interact with them in `main.py` as something like:
...
The user should be able to build your provided container without a hitch - it should come down to writing some additoinal code in the `main.py` script (the participant's implemented analysis) that produces some expected result. The instructions will vary slightly by container, but generally will look something like the following. First, the user will fork and clone the competition repo:
...
```
singularity shell -B analysis:/code/analysis analysis.img
```

In the case of large data, you might want to bind the data as well:

```
singularity shell -B analysis:/code/analysis -B data/mnt :/data/mnt analysis.img
```
